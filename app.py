...
with tab_overview:
    ...
    if len(cand):
        r = cand.iloc[0]
        actions.append(("Pricing", f"**Renegotiate or re-source {r['supplier_name']}**",
    ...
    st.caption("Bubble size = total spend. Hover for details. **Ideal suppliers sit top-left**: high performance score, low pricing premium.")
        ...
        if row.empty:
            ...
            with left:
                ...
                if note:
                    ...
                    if desc: st.markdown(f"*Team assessment: **{desc}***")
...
with tab_financial:
    ...
    st.caption("Quantifies the cost of supplier underperformance across three dimensions: pricing leakage, quality/rework cost, and delivery risk exposure.")
    ...
    f2.metric("Pricing Leakage",            _fmt_money(total_pricing),
              help="Avg premium above best comparable RFQ quote x estimated units")
    f3.metric("Est. Quality / Rework Cost", _fmt_money(total_quality),
              help="Defect rate x total spend x 0.5 rework cost factor")
    ...
    st.caption("Suppliers furthest to the right carry the highest combined financial risk. Stacked bars show the breakdown between pricing leakage and rework cost.")
        ...
        stacked = (
            ...
            .encode(
                ...
                color=alt.Color("type:N",
                                scale=alt.Scale(domain=["Pricing Leakage","Quality / Rework Cost"],
                                                range=["#FF7F0E","#D62728"]),
                                title="Cost Type",
    ...
    st.caption(
        "**High spend + low performance = highest priority.** "
        "Suppliers in the bottom-right quadrant represent your most urgent intervention targets."
    )
