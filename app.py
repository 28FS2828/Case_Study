...
- Adds Apex price debugging + makes entity matching more robust
"""
...
def parse_supplier_notes_robust(text: str) -> dict:
    ...
    while i < len(lines):
        ...
        # Pattern B block: sep, header, sep
        if is_sep(ln):
            # Find next non-empty line as header
            j = i + 1
            ...
            # Next, expect another separator soon; if not, still treat as header if it looks like a supplier line
            k = j + 1
            while k < len(lines) and not is_sep(lines[k].strip()):
                # allow a few blank lines
                if lines[k].strip():
                    # if a non-sep and non-empty line appears immediately, still ok; we'll treat following lines as body
                    break
                k += 1

            # Move k forward if we landed on sep line
            if k < len(lines) and is_sep(lines[k].strip()):
                body_start = k + 1
            else:
                body_start = j + 1

            # Capture until next separator line that starts a new block
            body_lines = []
            m = body_start
            while m < len(lines):
                if is_sep(lines[m].strip()):
                    # look ahead to see if a header exists after this separator; if yes, stop this block
                    # (prevents stopping at decorative lines inside body)
                    look = m + 1
            ...
            # Parse header into supplier + descriptor if possible
            supplier_raw = header
            descriptor = ""
            ...
            if supplier_key:
                ...
                # prune obvious "END OF NOTES" style blocks
    ...
    # Fallback Pattern A (older format) if nothing parsed
    if not notes:
        blocks = re.split(r"\n=+\n", str(text))
        for b in blocks:
            b = b.strip()
...
def render_full_note(note: dict):
    """
    Shows FULL supplier note content, with descriptor and nicely formatted entries.
    """
    ...
    # Render paragraphs and preserve dates/labels
    ...
    for para in re.split(r"\n\s*\n", body):
        ...
        # If a paragraph starts with something like "Email from X (date):", highlight header
        ...
        if head_match:
            ...
            # otherwise just render with line breaks
...
def process_raw(orders, quality, rfqs, notes_text):
    ...
    # If quality also has supplier_name sometimes, normalize it too (safe no-op if missing)
...
def try_load_local():
    for op, qp, rp, np in [
        ...
        # your mounted notes file path (for local execution environments)
...
# Merge supplier onto quality from orders (source of truth for supplier_name)
...
# IMPORTANT: normalize supplier_name again after numeric filtering (protects against stray whitespace)
...
with tab_overview:
    ...
    k1.metric(
        ...
        help="Avg price premium above best comparable market quote (same RFQ line), scaled across estimated units.",
    )
    k2.metric(
    ...
    st.caption("Clear, explainable recommendations tied to quantified impact (pricing leakage, quality cost, and concentration risk).")
...
""",
    ...
    # Pricing action: prioritize suppliers with leakage + ability to switch
    ...
    if len(cand):
        ...
        actions.append(
            (
                ...
                f"Est. {_fmt_money(float(r['estimated_overpay']))} annual leakage · {r['switchability']} switchability · {int(r['avg_alternatives'])} qualified alternative(s) on file",
    ...
    # Quality action: highest rework cost
    ...
    # Concentration action: spend share concentration
    ...
    if selected and selected != "— Choose a supplier —":
        ...
        if row_df.empty:
            ...
            with left:
                ...
                # Pricing vs market (same RFQ lines where possible)
                st.markdown("---")
...
with tab_decision:
    ...
    st.metric("Est. Annual Savings Available", _fmt_money(float(impact_dec["estimated_overpay"].sum())), help="Based on avg price premium above best comparable quote × estimated units")
    show_table(
        with_rank(
            format_for_display(
...
# ═══════════════════════════════════════════════════════
# TAB 4 · PERFORMANCE TRENDS (SIMPLIFIED)
# ═══════════════════════════════════════════════════════
with tab_trends:
    ...
    if "order_date" not in orders.columns:
        ...
        with cR:
            st.selectbox("Window", [6, 12, 18, 24], key="trend_months")
        ...
        # defect is overall (not monthly) but still useful as quick KPI
        ...
        k["spend_k"] = (k["spend"] / 1000).round(0)
        ...
        if focus == "On-Time Trend":
            ...
            st.caption("Simple line chart. Higher is better. Each line is a supplier. This is the one chart most executives need.")
            ...
            st.caption("Count of late orders per month. Higher bars indicate reliability issues.")
            ...
            st.caption("How spend is shifting over time. Useful for spotting concentration changes without confusing stacked visuals.")
        ...
        st.markdown("---")
        with st.expander("Debug: why might Apex prices be missing?"):
            st.write("Supplier names in supplier_master:", supplier_master["supplier_name"].tolist())
...
with tab_financial:
    ...
    st.caption("Quantifies the cost of supplier underperformance across pricing leakage, quality/rework cost, and delivery-risk exposure.")
    ...
    if not risk_plot.empty:
        ...
        stacked = (
            ...
            .encode(
                ...
                tooltip=[alt.Tooltip("supplier_name:N", title="Supplier"), alt.Tooltip("type:N", title="Cost Type"), alt.Tooltip("cost:Q", title="Est. Cost ($)", format="$,.0f")],
            )
...
"""
    ...
    with st.expander("Debug — internal notes coverage"):
        ...
        for s in all_suppliers:
            if normalize_supplier_key(s) not in supplier_notes:
