...
(Executive polished version — baseline preserved, fixes layered in)

Fixes added without removing baseline sections:
1) Robust supplier notes parsing + supplier_key matching (fixes notes not populating for Apex, etc.)
2) RFQ avg quote price + pricing impact calculations aligned using supplier_key (fixes Apex avg_price missing)
3) Executive Overview: wider action badges + clearer "why" context + simpler terminology (Estimated Overpay)
4) Performance Trends: supplier selector changed to dropdown + "All suppliers" option + window max increased
"""
...
/* Executive action badge: widen + wrap */
.hoth-badge {
...
.hoth-card-title { font-weight:700; font-size:1.02rem; margin-bottom:4px; }
.hoth-card-sub { color:#6b7280; font-size:0.88rem; line-height:1.25rem; }
.hoth-why { margin-top:8px; font-size:0.92rem; line-height:1.35rem; color:#111827; }
.hoth-why b { font-weight:800; }
...
def apply_entity_resolution(df: pd.DataFrame, col: str, manual_map: dict = None) -> pd.DataFrame:
    ...
    # also attach supplier_key after resolution (important!)
...
def rfq_competitiveness(rfqs_df: pd.DataFrame, cat: str) -> pd.DataFrame:
    ...
    r = rfqs_df.copy()
    if "supplier_key" not in r.columns:
        r = add_supplier_key(r, "supplier_name")

    r = _add_part_category(r)
    ...
    r["_lk"] = r[lk].astype(str).apply(_norm_text if lk == "part_description" else lambda x: str(x).strip())
...
def build_pricing_impact(master: pd.DataFrame, rfqs_df: pd.DataFrame, cat: str) -> pd.DataFrame:
    ...
    # simpler label: "estimated_overpay"
...
# ─────────────────────────────────────────────
# SUPPLIER NOTES (ROBUST + FULL BODY)
# ─────────────────────────────────────────────
def parse_supplier_notes(text: str) -> dict:
    """
    Robust parser for notes separated by lines of '='.
    Supports:
      ========
    ...
    Returns:
      {supplier_key: {"descriptor": str, "body": str}}
    """
    notes = {}
    ...
    while i < len(lines):
        if is_sep(lines[i]):
            ...
            supplier_key = normalize_supplier_key(supplier_raw)
            supplier_key = MANUAL_KEY_MAP.get(supplier_key, supplier_key)

            body_lines = []
            k = j + 1
            while k < len(lines) and not is_sep(lines[k]):
                body_lines.append(lines[k])
                k += 1

            body = "\n".join(body_lines).strip()
            if supplier_key:
                notes[supplier_key] = {"descriptor": desc, "body": body}
    ...
    if not notes:
        for b in re.split(r"\n=+\n", str(text)):
            ...
            supplier_raw = m.group(1).strip()
            desc = m.group(2).strip()
            supplier_key = MANUAL_KEY_MAP.get(normalize_supplier_key(supplier_raw), normalize_supplier_key(supplier_raw))
            body = "\n".join(b.splitlines()[1:]).strip()
            notes[supplier_key] = {"descriptor": desc, "body": body}

    return notes

def note_snippet(notes: dict, supplier_name: str) -> str:
    k = MANUAL_KEY_MAP.get(normalize_supplier_key(supplier_name), normalize_supplier_key(supplier_name))
    n = notes.get(k, {})
    if not n: return ""
    parts = []
    if n.get("descriptor"):
...
def get_full_note(notes: dict, supplier_name: str) -> dict:
    k = MANUAL_KEY_MAP.get(normalize_supplier_key(supplier_name), normalize_supplier_key(supplier_name))
    return notes.get(k, {})
...
def process_raw(orders, quality, rfqs, notes_text):
    # entity resolution must run BEFORE adding supplier_key
    ...
    # make sure supplier_key is present
    orders = add_supplier_key(orders, "supplier_name")
    rfqs = add_supplier_key(rfqs, "supplier_name")
...
def try_load_local():
    for op, qp, rp, np in [
        ...
        # support your uploaded sandbox path too
        ...
        try:
            o = pd.read_csv(op); q = pd.read_csv(qp); r = pd.read_csv(rp)
...
# ----- FIX: avg quote price aggregation aligned via supplier_key -----
...
if "supplier_key" not in ap.columns:
    ap = add_supplier_key(ap, "supplier_name")

# build a stable mapping supplier_key -> display supplier_name (prefer orders' canonical names)
orders_map = add_supplier_key(orders, "supplier_name")[["supplier_name","supplier_key"]].dropna()
key_to_name = (orders_map.groupby("supplier_key")["supplier_name"]
               .agg(lambda s: sorted(set([x for x in s if isinstance(x,str) and x.strip()]), key=lambda x:(-len(x), x))[0] if len(s) else "")
               .to_dict())
...
ap_by_key["supplier_name"] = ap_by_key["supplier_key"].map(key_to_name).fillna(ap_by_key["supplier_key"])

# collapse to supplier_name (canonical)
ap_final = ap_by_key.groupby("supplier_name", dropna=False)["avg_price"].mean().reset_index()
ap_final["avg_price"] = ap_final["avg_price"].round(2)

supplier_master = (
    spend.merge(on_time, on="supplier_name", how="left")
         .merge(defects, on="supplier_name", how="left")
         .merge(ap_final, on="supplier_name", how="left")
...
with tab_overview:
    ...
    # merge switchability back by supplier_key -> supplier_name (safe)
    # best effort: rebuild supplier_key on impact_exec and sw_exec
    impact_exec = add_supplier_key(impact_exec, "supplier_name")
    sw_exec = sw_exec.copy()
    if "supplier_key" not in sw_exec.columns and "supplier_name" in sw_exec.columns:
        sw_exec = add_supplier_key(sw_exec, "supplier_name")

    if "supplier_key" in impact_exec.columns and "supplier_key" in sw_exec.columns:
        impact_exec = impact_exec.merge(sw_exec, on="supplier_key", how="left")
    else:
        impact_exec["avg_alternatives"] = 0.0
        impact_exec["switchability"] = "LOW"

    impact_exec["avg_alternatives"] = impact_exec.get("avg_alternatives", 0.0).fillna(0.0)
    impact_exec["switchability"] = impact_exec.get("switchability", "LOW").fillna("LOW")
    ...
    # KPI Strip (simpler terms)
    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Estimated Overpay", _fmt_money(total_overpay),
    ...
    # Priority Actions (wider badge + more context)
    st.subheader("Priority Actions")
    st.caption("Recommendations below include the business reason and the supporting numbers.")
...
<div class="hoth-card">
  <div style="display:flex; gap:14px; align-items:flex-start;">
    <div class="hoth-badge" style="background:{color};">{label.upper()}</div>
    <div style="flex:1;">
      <div class="hoth-card-title">{title}</div>
      <div class="hoth-card-sub">{detail}</div>
      <div class="hoth-why"><b>Why this matters:</b> {why}</div>
    </div>
  </div>
</div>
""",
    ...
    if len(cand):
        ...
        actions.append((
            ...
            f"{r['supplier_name']} averages {_fmt_money_2(float(r['avg_delta_vs_best']))}/unit above the best quote on comparable RFQ lines. Alternatives exist, so savings is achievable."
        ))
    ...
    if len(tmp) and float(tmp.iloc[0]["defect_rate"]) > 0:
        ...
        actions.append((
            ...
            f"Defects create real rework cost and schedule drag. Add containment (inspection) + root cause and gate volume increases until fixed."
        ))
    ...
    if len(tmp2) and float(tmp2.iloc[0]["spend_share_pct"]) >= 20:
        ...
        actions.append((
            ...
            f"High concentration means one disruption can halt deliveries. A backup reduces outage risk even if primary remains preferred."
        ))
    ...
    # Positioning Matrix (unchanged)
    st.subheader("Supplier Positioning Matrix")
    st.caption("Bubble size = total spend. Hover for details. **Ideal suppliers sit top-left**: high performance score, low overpay.")
    ...
    matrix = (
        ...
        .encode(
            x=alt.X("avg_delta_vs_best:Q",
                    title="Avg Overpay vs Best Quote ($/unit) — lower is better",
    ...
    ref_line = alt.Chart(pd.DataFrame({"y":[75]})).mark_rule(
...
with tab_intel:
    ...
    if selected and selected != "— Choose a supplier —":
        ...
        if row.empty:
            ...
            if "Quality" in risk: st.error( f"STOP — {risk}: Review quality history before sending this RFQ.")
            elif "Delivery" in risk: st.warning(f"CAUTION — {risk}: Add schedule buffer to this order.")
            elif "Cost" in risk: st.info( f"NOTE — {risk}: Not price-competitive. Solicit at least 2 alternatives.")
            ...
            with right:
                ...
                if not sup_q.empty:
                    ...
                    show_table(sup_q_show.sort_values("inspection_date", ascending=False) if "inspection_date" in sup_q_show.columns else sup_q_show, max_rows=8)
                else:
...
with tab_decision:
    ...
    if sc_orders.empty:
        ...
        sc_ap = (sc_rp.groupby("supplier_name", dropna=False)["quoted_price"].mean().reset_index(name="avg_price")
                 if not sc_rp.empty else pd.DataFrame({"supplier_name":sc_spend["supplier_name"],"avg_price":0.0}))
        ...
        decision_kpi["performance_score"] = (
            decision_kpi["on_time_rate"]*wd + (100-decision_kpi["defect_rate"])*wq + decision_kpi["price_score"]*wc
        ).round(1)
    ...
    st.metric("Est. Annual Savings Available", _fmt_money(float(impact_dec["estimated_overpay"].sum())),
              help="Based on avg overpay vs best comparable quote × estimated units")
...
with tab_trends:
    ...
    st.caption("Delivery reliability, quality, and spend allocation over time. Use these for weekly ops reviews and board reporting.")

    if "order_date" not in orders.columns:
        st.warning("No `order_date` column found — trends unavailable.")
    else:
        # NEW: dropdown selector + All suppliers + larger window
        ...
        with cA:
            st.selectbox(
                ...
                key="trend_supplier_mode",
            )
        with cB:
            st.selectbox(
                "Window (months):",
                options=[6, 12, 18, 24, 36, 48, 60],
                key="trend_window_months",
            )
        ...
        months = int(st.session_state["trend_window_months"])
        ...
        ot_trend = ot_trend[ot_trend["month"] >= start_cut].copy()

        # ── Chart 1: Delivery Heatmap ──
        st.markdown("#### Delivery Reliability — Monthly On-Time Rate Heatmap")
        st.caption("Each cell shows the % of orders delivered on time that month. Red = chronic delays, green = reliable. Blank = no orders that month.")
        ...
        if ot_trend.empty:
            ...
            heatmap = (
                ...
                .encode(
                    ...
                    y=alt.Y("supplier_name:N", title=None,
                             sort=alt.SortField("otr_pct", order="descending")),
                    color=alt.Color("otr_pct:Q",
                                    title="On-Time %",
                ...
                .properties(height=max(130, len(tf) * 48))
            )
            text_ov = (
                alt.Chart(heat)
        ...
        st.caption("Each dot = one late order. Size = PO value. Persistent dots for a supplier signal a systemic reliability problem.")
        ...
        late_orders = late_orders[late_orders["month"] >= start_cut].copy()
        ...
        st.caption("Overall rejection rate across all inspections. The orange line marks the 5% corrective-action threshold.")

        defect_bar_data = defects[defects["supplier_name"].isin(tf)].sort_values("defect_rate", ascending=False).copy()
        defect_bar_data["above"] = defect_bar_data["defect_rate"].apply(
            lambda x: "Above threshold" if x > 5 else "Within threshold"
        )
        ...
        if not defect_bar_data.empty:
            dbar = (
                ...
                .encode(
                    ...
                    color=alt.Color("above:N",
                                    scale=alt.Scale(domain=["Above threshold","Within threshold"],
                                                    range=["#D62728","#2CA02C"]),
            ...
            tline = alt.Chart(pd.DataFrame({"y":[5]})).mark_rule(
                color="#FF7F0E", strokeDash=[6,3], strokeWidth=2
            ).encode(y="y:Q")
        ...
        st.caption("Tracks how your sourcing concentration has shifted over time. Rising share from a single supplier signals growing dependency risk.")
        ...
        spend_trend = spend_trend[spend_trend["month"] >= start_cut].copy()
        ...
        if not ms.empty:
            area = (
                ...
                .encode(
                    x=alt.X("month:T", title="Month", axis=alt.Axis(format="%b %y")),
                    y=alt.Y("spend_k:Q", title="Spend ($K)", stack="normalize",
...
with tab_financial:
    ...
    st.caption("Quantifies the cost of supplier underperformance across pricing (overpay), quality/rework cost, and delivery risk exposure.")
    ...
    total_overpay = float(impact_df["estimated_overpay"].sum())
    total_quality = float(impact_df["defect_cost"].sum())
    total_risk = total_overpay + total_quality

    # KPIs
    f1, f2, f3, f4 = st.columns(4)
    f1.metric("Total Quantified Risk", _fmt_money(total_risk),
              help="Estimated overpay + est. quality cost combined")
    f2.metric("Estimated Overpay", _fmt_money(total_overpay),
              help="Avg overpay vs best comparable quote × estimated units")
    ...
    st.caption("Suppliers furthest to the right carry the highest combined financial risk. Stacked bars show the breakdown between estimated overpay and rework cost.")
    ...
    if not risk_plot.empty:
        ...
        melt["type"] = melt["type"].map({
            "estimated_overpay": "Estimated Overpay",
        ...
        stacked = (
            ...
            .encode(
                ...
                color=alt.Color("type:N",
                                scale=alt.Scale(domain=["Estimated Overpay","Quality / Rework Cost"],
                                                range=["#FF7F0E","#D62728"]),
                                title="Cost Type",
    ...
    st.caption("High spend + low performance = highest priority intervention targets.")
    ...
    st.caption("Dashed line = 75 performance score. Bottom-right = large spend, weak performance — immediate action required.")

    st.markdown("---")

    # ── Chart 3: Pricing Premium Distribution ──
    st.markdown("#### Quote Competitiveness — Overpay vs Best Quote")
    st.caption("How much more (on avg) does each supplier charge versus the lowest competing quote for the same RFQ line? Zero is ideal.")
    ...
    if not price_plot.empty:
        price_bar = (
            ...
            .encode(
                x=alt.X("avg_delta_vs_best:Q",
                        title="Avg Overpay vs Best Quote ($/unit)",
                        scale=alt.Scale(zero=True)),
                y=alt.Y("supplier_name:N", sort="-x", title=None),
    ...
    with st.expander("Methodology & data notes"):
        st.markdown("""
**Estimated Overpay**
Overpay is computed *within the same RFQ line* (apples-to-apples — never compares prices across different parts).
Estimated Units = total spend ÷ avg quoted price. Overpay = (Avg overpay per unit) × (Estimated units).
...
""")
    ...
    with st.expander("Debug — column names"):
        ...
        st.write("Note keys parsed:", list(supplier_notes.keys())[:25])
